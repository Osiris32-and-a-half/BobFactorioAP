...
from .FactorioOptions import TechTreeLayout

if TYPE_CHECKING:
    from . import FactorioBobs, FactorioScienceLocation
...
def get_shapes(world: "FactorioBobs") -> Dict["FactorioScienceLocation", Set["FactorioScienceLocation"]]:
    ...
    if layout == TechTreeLayout.option_single:
        ...
        while len(locations) >= 1: #Loop through all remaining techs
            ...
            while (True):
                list.append(current_choices[int(rand_num % len(current_choices))]) #Take one of the already processed techs as its prerequisite.
            ...
            prerequisites[victim] = set(list) #set the list as is apropiate.
    else:

...
class FactorioContext(CommonContext):
    ...
    def on_user_say(self, text: str) -> typing.Optional[str]:
        ...
        if self.slot in self.player_names:
            self.print_to_game(f"{self.player_names[self.slot]}: {text}")
        else:
            self.print_to_game(f"ERROR PLEASE REPORT STATE ({self.slot}: {self.player_names}): {text}")
        return text
...
async def factorio_spinup_server(ctx: FactorioContext) -> bool:
    savegame_name = Utils.user_path("Archipelago.zip")
    if not os.path.exists(savegame_name):
        logger.info(f"Creating savegame {savegame_name}")
        subprocess.run((
...
settings: FactorioSettings = get_settings().factorio_modpacks_options
